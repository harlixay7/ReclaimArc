...
use spacextract_core::fault::CrashPoint;
...
fn child_env(dir: &Path, point: CrashPoint, mode: ExtractionMode) -> std::collections::HashMap<String, String> {
    ...
    env.insert("SPACEEXTRACT_FAULT_AT".to_string(), point.as_str().to_string());
    env.insert(
        "SX_MODE".to_string(),
        match mode {
    ...
    env.insert("SPACEEXTRACT_TEST_FREE_SPACE".to_string(), "100000000000".to_string());
    env.insert("SPACEEXTRACT_APP_DATA".to_string(), dir.join("appdata").to_string_lossy().into_owned());
    env
}
...
/// Find the newest journal in `<archive_dir>/.spacextract/*/job.db`.
fn find_journal(dir: &Path) -> PathBuf {
    let state = dir.join("archive").join(".spacextract");
...
fn crash_and_resume(point: CrashPoint, mode: ExtractionMode) {
    ...
    let journal = spacextract_journal::JobJournal::open(&journal_path).expect("journal must reopen");
    ...
    match point {
        CrashPoint::AfterPartialWrite | CrashPoint::AfterOutputFlush => {
            ...
            assert!(
                !spacextract_core::state::is_committed(u0.state),
            ...
            assert!(ranges.iter().all(|r| r.state == spacextract_journal::models::RangeState::Active));
        }
        CrashPoint::AfterRename => {
            // Finals exist but must be adoptable (verified, not committed).
            let u0 = units.iter().find(|u| u.seq == 0).unwrap();
            assert!(!spacextract_core::state::is_committed(u0.state));
        ...
        CrashPoint::BeforeHolePunch | CrashPoint::DuringHolePunch | CrashPoint::BeforeReclaimedCommit => {
            ...
            if point == CrashPoint::BeforeHolePunch {
                ...
                assert!(ranges.iter().all(|r| r.state == spacextract_journal::models::RangeState::ReclaimIntent));
                let file = spacextract_platform::sparse::open_for_reclaim(
                    &journal.volumes().unwrap()[0].path,
                )
                .unwrap();
let allocated = spacextract_platform::sparse::query_allocated_ranges(
                    &file,
                    &journal.volumes().unwrap()[0].path,
                    0,
...
// Resume WITHOUT fault injection and complete.
    ...
    let journal = spacextract_core::recovery::prepare_resume(&journal_path, None).expect("resume prep");
    let mut backend = spacextract_archive::backend_for(&meta.archive_path).unwrap();
    let info = backend.inspect(&spacextract_archive::OpenOptions::default()).unwrap();
    let name_map: HashMap<u64, String> = info
        .entries
        .iter()
        .filter(|e| !e.is_directory)
        .map(|e| {
            let safe = spacextract_core::paths::validate_entry(&e.name, false).unwrap();
            (e.index, safe.relative())
        })
        .collect();
    let (tx, rx) = mpsc::channel();
    let mut job = spacextract_core::JobJob {
    ...
    if mode == ExtractionMode::LowSpace {
        ...
        let file = spacextract_platform::sparse::open_for_reclaim(&vol[0].path).unwrap();
        let allocated = spacextract_platform::sparse::query_allocated_ranges(&file, &vol[0].path, 0, std::fs::metadata(&vol[0].path).unwrap().len()).unwrap();
        let total_alloc: u64 = allocated.iter().map(|r| r.len).sum();
        ...
        let ranges = journal.packed_ranges().unwrap();
        assert!(ranges.iter().all(|r| r.state == spacextract_journal::models::RangeState::Reclaimed));
    }
}
...
fn low_space_extraction_succeeds_where_normal_fails() {
    ...
    std::env::set_var("SPACEEXTRACT_TEST_FREE_SPACE", free_observed.to_string());
    std::env::set_var("SPACEEXTRACT_APP_DATA", dir.path().join("appdata"));
    ...
    let err = engine
        ...
        .unwrap_err();
    assert!(matches!(err, spacextract_core::CoreError::Infeasible(_)), "normal must be infeasible: {err:?}");
    ...
    let file = spacextract_platform::sparse::open_for_reclaim(&paths[0]).unwrap();
    let allocated =
        spacextract_platform::sparse::query_allocated_ranges(&file, &paths[0], 0, std::fs::metadata(&paths[0]).unwrap().len()).unwrap();
    let total_alloc: u64 = allocated.iter().map(|r| r.len).sum();
...
fn path_traversal_is_rejected() {
    ...
    std::env::set_var("SPACEEXTRACT_TEST_FREE_SPACE", "100000000000");
    std::env::set_var("SPACEEXTRACT_APP_DATA", dir.path().join("appdata"));
    ...
    let err = engine
        ...
        .unwrap_err();
    assert!(
        matches!(err, spacextract_core::CoreError::Precondition(_)),
...
fn pause_then_resume_completes() {
    ...
    let journal_path = find_journal(dir.path());
    let j = spacextract_journal::JobJournal::open(&journal_path).unwrap();
    let meta = j.job_meta().unwrap();
    let _ = meta;

// Resume to completion.
    drop(job);
    let journal = spacextract_core::recovery::prepare_resume(&journal_path, None).unwrap();
    for entry in std::fs::read_dir(&dest).unwrap().flatten() {
        eprintln!("PAUSE TEST: dest after prepare_resume has {}", entry.file_name().to_string_lossy());
    }
    let mut backend = spacextract_archive::backend_for(&paths[0]).unwrap();
    let info = backend.inspect(&spacextract_archive::OpenOptions::default()).unwrap();
    let name_map: HashMap<u64, String> = info
        .entries
        .iter()
    ...
    let mut job2 = spacextract_core::JobJob {
...
fn source_modification_is_detected() {
    ...
    std::fs::create_dir_all(&archive_dir).unwrap();
    std::fs::create_dir_all(&dest).unwrap();
    let files = vec![FixtureFile::new("a.bin", &vec![0xAA; 50_000])];
    let paths = write_rar(&archive_dir, "src", &files, &FixtureOptions::default()).unwrap();
    ...
    std::env::remove_var("SPACEEXTRACT_FAULT_AT");

    // Tamper with the archive BEFORE resuming.
    std::fs::write(&paths[0], b"tampered").unwrap();

    let journal_path = find_journal(dir.path());
    let err = spacextract_core::recovery::prepare_resume(&journal_path, None).unwrap_err();
    assert!(
        matches!(err, spacextract_core::CoreError::Failed { .. }),
